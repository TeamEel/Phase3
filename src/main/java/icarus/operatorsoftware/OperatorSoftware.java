...
/**
 * Emulates the software component of the nuclear reactor. In addition, it owns
 * and manages the hardware components of the reactor. All interaction with the
 * system from the user goes through this level.
 * 
 * @author Team Haddock
 * 
 */
public class OperatorSoftware {
	...
	/**
	 * Default constructor for OperatorSoftware, typically used upon starting a
	 * new game.
	 */
	 ...
	 * 
	 * @param component
	 *            Value of the enum of which component to retrieve the temperature
	 ...
	 * 
	 * @param component
	 *          Value of the enum of which component to retrieve the pressure
	 ...
	 * 				Thrown when attempting to fix a functional component
	 */
	 ...
	 * 				Thrown when attempting to fix a functional component
	 */
	public void fix(Component component, int pumpNum) throws InvalidComponentException, FixAlreadyUnderwayException, InvalidPumpException, NoFixNeededException {
		if (!fixUnderway()) {
			if (component == Component.WATERPUMP) {
				if (pumpNum == 0 || pumpNum == 1)
	...
	/**
	 * Progresses time in the system. All time dependent activities occur such
	 * as changes in temperature, pressure, water levels, power output and
	 * equiptment failure and fixes.
	 */
	public void next() {
	...
	public boolean fixUnderway() {
		return condenser.getRepairing() || reactor.getRepairing() || turbine.getRepairing() || condenserPump.getRepairing() || waterPump[0].getRepairing() || waterPump[1].getRepairing();
	}
	...
	/**
	 * Checks whether the game has ended - if the temperature in the reactor has
	 * exceeded the max
	 * 
	...
	/**
	 * Places all objects in the reactor + Player in a hashtable, for saving
	 * purposes
	 * 
	 * @return a hashtable
	 */
