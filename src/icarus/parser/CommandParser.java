...
/**
 *
 * CommandParser class holds the processing of all commands, registered through the console.
 * It is also the main starting point of the game.
 * 
 * @author  Team Haddock
 * @version 2012/2013
 */
...
public class CommandParser {
    ...
    /**
     * The welcome() method is called when a player starts a fresh game. It introduces the background story,
     * shows the player the available commands and gives the player some instruction as to how to play the game.
     */
    public void welcomeNewPlayer(){
        ...
        System.out.println(String.format("\"Oh and %s, don't forget to progress your reactor by using the \'next\'" + '\n' +
        		"command after you've made all the modifications to the systems components that you want!" + '\n' +
        		"The display will let you know how the reactor ,is doing every time you progress with \'next\'\"",cf.getPlayerName()));
    ...
    /**
     * The play() method sets up the parser and the loop. The finished local variable serves as a
     * flag to the while loop. A Scanner object is used for reading input. A case for each command
     * is defined in the switch expression.
     */
    public void play(){
        String userResponse="";
        boolean finished = false;
    ...
    /**
     * The method processes each line of input. First word is considered the command, and if there are
     * second and third they are the objects of the command.
        ...
        if(tokenizer.hasNext()) {
            ...
            if(tokenizer.hasNext()) {
                ...
                if(tokenizer.hasNext()){
                    ...
                    } else  word3 = tokenizer.next().toLowerCase();
    ...
    public void updateDisplay() {
    	...
    	try{
    		...
    		System.out.println(String.format("%-15s%15s%15s", "Water level", df.format(cf.waterLevel(Component.REACTOR))+" cm", df.format(cf.waterLevel(Component.CONDENSER))+" cm" ));
    		System.out.println(String.format("%-15s%15s%15s", "Pressure level", df.format(cf.pressure(Component.REACTOR))+" kPa", df.format(cf.pressure(Component.CONDENSER))+" kPa"));
	        System.out.println(String.format("%-15s%15s%15s", "Temperature", df.format(cf.temperature(Component.REACTOR))+" K", df.format(cf.temperature(Component.CONDENSER))+" K"));
        ...
        if(cf.doFix()) System.out.println("A component has been repaired.");
        reportFailures();
    }
    ...
    public void reportFailures(){
    	...
    	if(cf.fixUnderway()) System.out.println("Time remaining on current fix: " + cf.getFixTime());
    }
    ...
    public void gameOverScenario(){
        ...
        System.out.println(String.format("Your reactor has failed, %s !! It's game over for you. Good luck next time..", playerName));
        System.out.println();
        System.out.println();
