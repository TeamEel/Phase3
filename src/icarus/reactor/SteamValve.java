...
/**
 * Steam Valve system that controls the flow of steam between the reactor,
 * turbine and condenser. Consists of two valves: the first flows via the
 * turbine, the second does not.
 * 
 * @author Team Haddock
 * 
 */
public class SteamValve implements Serializable {
	 ...
	 * 
	 * @param mComponentIn
	 *            The MajorComponent pushing steam into the one-way valve system
	 * @param turbine
	 *            The turbine which the steam passes over
	 * @param mComponentOut
	 *            The MajorComponent receiving steam out of the valve system
	 */
	...
	/**
	 * Moves steam from the in component to the out component according to
	 * pressure.
	 */
	public void equalise() {
		...
		if ((turbine.getFunctional() && open1) || open2) {
			...
			if (pressureDiff > 0) {
				...
				double amount = mCompIn.drainSteam((mCompIn.getHeight() - mCompIn.getWaterLevel()) * (pressureDiff / 2) / mCompIn.getTemperature());
	 ...
	 * 
	 * @param valveID
	 *            ID of the valve to open
	 * @throws InvalidValveException
	 *             Thrown when a bad ID is given
	 * @throws AlreadyAtStateException
	 *             Thrown if valve is already open
	 */
	 ...
	 * 
	 * @param valveID
	 *            ID of the valve to close
	 * @throws InvalidValveException
	 *             Thrown when a bad ID is given
	 * @throws AlreadyAtStateException
	 *             Thrown if valve is already closed
	 */
	 ...
	 * @param valveID
	 *            ID of valve open status to return
